...
public abstract class UpdateProfile {
    ...
    public static void uploadImageToStorage(InputStream inputStream){
        ...
        StorageReference profileStorageRef = storage.getReference().child(Constants.PROFILE_REFERENCE + Objects.requireNonNull(user).getUid());
    ...
    private static void updateUserImage(){
        ...
        }).addOnFailureListener(e -> Log.d("UpdateProfile","updateUserImage: onFailure"));
    }
    ...
    public static void showDeleteImageDialog(Context context){
        ...
        builder.setPositiveButton(R.string.ok, (dialog, whichButton) -> deleteImageFromStorage());
    ...
    private static void deleteUserImage(){
        FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();
        DatabaseReference databaseReference = FirebaseDatabase.getInstance(Constants.DB_URL).getReference().child(Constants.USERS_REFERENCE);
        databaseReference.child(Objects.requireNonNull(user).getUid()).child(Constants.IMAGE_URL_REFERENCE).setValue("");
    ...
    private static void updateUsername(String new_username){
        FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();
        DatabaseReference databaseReference = FirebaseDatabase.getInstance(Constants.DB_URL).getReference().child(Constants.USERS_REFERENCE);
        databaseReference.child(Objects.requireNonNull(user).getUid()).child(Constants.USERNAME_REFERENCE).setValue(new_username);
    }
